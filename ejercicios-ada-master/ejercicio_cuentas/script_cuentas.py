...
def aplicar_transferencias(cuentas, nro_de_cuenta_origen, nro_de_cuenta_destino, monto):
    for cuenta in cuentas:
        if cuenta[0] == nro_de_cuenta_origen:
            cuenta[1] = str(float(cuenta[2]) + monto )
        break
    return cuentas
    
def transferencias(cuentas):
    archivo = open("transferencias.csv")
    primera_linea = True
    for linea in archivo:
        if not primera_linea:
            nro_de_cuenta_origen, nro_de_cuenta_destino, monto =linea.replace('\n', '').split(',')
            transferencia_monto_float = (monto)
            cuentas_actualizadas = aplicar_transferencias(cuentas)
            imprimir_cuentas(cuentas_actualizadas)
